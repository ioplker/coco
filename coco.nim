...
proc handle_testament_spec(filename: string): string =
    ## Extracts `testament`'s `define` variables from test's spec
    ## Generates string like `-d:someFlag -d:someOtherFlag`
    let spec = parseSpec(filename)
...
proc compile*(target="tests/**/*.nim", nimcache=nimcache_default, verbose=false, options= "") =
    ...
    for nimfile in walkGlob(target):
        ...
        # Extract `testament`'s `define` variables from test's spec
        when defined(useTestament):
            let definedVarsString = handle_testament_spec(nimfile)
            exec(fmt"nim {nim_args} {definedVarsString} --nimcache={cache_folder} --debugger:native --passC:--coverage --passL:--coverage c " & nimfile)
        else:
...
proc trace*(target: string) =
    ...
    for nimfile in walkGlob(target):
        ...
        #* NOTE: Commented out cause this removes files, that called several times - better to put this into cleanup
...
proc remove_absent_files(fileinfo = fileinfo, lcov_args: string) =
  ## Removes non-existent files from tracefile
  ## Useful for removing 3rd-party libs and tests (tests for some reason are placed at root)
  echo &"Removing absent files from {fileinfo}..."

  var files_to_remove: seq[string]
  for line in fileinfo.lines:
      if line.startsWith("SF:"):
          let filename = line[3..^1]
          if not fileExists(filename):
              files_to_remove.add(filename)

  for filename in files_to_remove:
      exec(fmt"""lcov {lcov_args} --remove {fileinfo} "{filename}" -o {fileinfo}""")
...
proc cleanup_report*(fileinfo = fileinfo, cov: string, verbose=false, branch=false) =
    ...
    remove_absent_files(fileinfo, lcov_args)
